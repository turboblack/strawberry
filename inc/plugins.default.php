...
global $row, $config;
	...
	if ($config['database'] != 'txtsql'){
		return '<fieldset id="sticky"><legend>'.t('Закрепить новость').'</legend><label for="sticky_post"><input type="checkbox" id="sticky_post" name="sticky_post" value="on"'.($row['sticky'] ? ' checked' : '').'>&nbsp;'.t('Закрепить эту новость?').'</label></fieldset>';
	}
}
...
global $row, $config;
	...
	$result  = '<fieldset id="date"><legend>'.t('Дата').'</legend>';
	$result .= '<input type="text" name="day" maxlength="2" style="width: 19px;" value="'.langdate('d', $time).'" title="'.t('День').'">';
    ...
    $result .= '@<input type="text" name="hour" maxlength="2" style="width: 19px;" value="'.langdate('H', $time).'" title="'.t('Час').'">';
    $result .= ':<input type="text" name="minute" maxlength="2" style="width: 19px;" value="'.langdate('i', $time).'" title="'.t('Минута').'">';
    $result .= ':<input type="text" name="second" maxlength="2" style="width: 19px;" value="'.langdate('s', $time).'" title="'.t('Секунда').'">';
    $result .= '</fieldset>';
...
return '<fieldset id="url"><legend>'.t('УРЛ (при желании)').'</legend><input type="text" size="42" name="url" value="'.$row['url'].'"></fieldset>';
}
...
global $PHP_SELF, $config;
	...
	echo '<div class="panel">'.t('Окно "urls.ini" показывает и даёт возможность настроить вид УРЛов. <a onClick="javascript:Help(\'rufus\')" href="#">О тегах, хитростях и само Писание :) смотрите в тут</a>. После редактирования нажмите "%save".', array('save' => t('Сохранить urls.ini'), 'make' => t('Создать .htaccess'))).'</div>';
?>
