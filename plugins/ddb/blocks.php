...
global $PHP_SELF, $QUERY_STRING;
    ...
    $self = $PHP_SELF.'?plugin=ddb'.cute_query_string($QUERY_STRING, array('action', 'block', 'category', 'plugin'));
...
<tr <?=cute_that(); ?>>
<td style="padding: 5px;"><b><a href="<?=$self; ?>&action=category"><?=t('Новая категория'); ?></a></b></td>
</tr>
<tr <?=cute_that(); ?>>
<td style="padding: 5px;"><b><a href="<?=$self; ?>"><?=t('Новый блок'); ?></a></b></td>
</tr>
<?=$list_directory; ?>
</table>
...
<? } ?>
<img border="0" src="skins/images/help_small.gif" align="absmiddle">&nbsp;<a onClick="javascript:Help('ddb')" href="#"><?=t('Что такое Drag\'n\'Drop Blocks и с чем их едят?'); ?></a>
<br />
<? if ($_GET['action'] != 'category'){ ?>
<?=t('PHP и HTML разрешены. Форматирования, как в новостях нет.'); ?><br />
<textarea name="content" style="height: 300;"><?=htmlspecialchars(file_read(blocks_directory.'/'.$_GET['block'].'.block')); ?></textarea>
...
<? if ($_GET['block'] or $_GET['category']){ ?>
...
function call_ajax(that){
	new Ajax.Updater(
	    {success: 'result'},
	    '<?=$_SERVER['PHP_SELF']; ?>?plugin=ajax&call=ddb',
	    {
...
<? foreach (run_filters('constructor-variables', array()) as $k => $v){ ?>
<tr <?=cute_that(); ?>>
<td>$<?=$k; ?>
<td>=
<td><?=$v[1]; ?>
<? } ?>
<tr>
<td colspan="3"><input type="submit" value=" <?=t('Сгенерировать'); ?>">
<tr>
...
<? foreach (run_filters('constructor-functions', array()) as $k => $v){ ?>
<tr <?=cute_that(); ?>>
<td> &lt;?=<?=$k; ?>(); ?&gt;<br /><small><?=function_help($k, t('помощь по работе функции')); ?></small>
<? } ?>
</table>
...
function ddb_save_positions(){
	...
	if (cute_get_rights('ddb', 'write') and $_GET['ddb'] == 'save'){
...
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/core.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/events.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/tool-man/css.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/coordinates.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/drag.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/dragdrop.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/dragsort.js"></script>
<script language="JavaScript" type="text/javascript" src="<?=$config['http_script_dir']; ?>/plugins/ddb/cookies.js"></script>
<script>
...
<?
    ...
    if (cute_get_rights('ddb', 'read') and $_GET['ddb'] == 'edit'){
