...
global $member, $title, $short_story, $url, $full_story, $added_time, $mod, $id, $category;
    ...
    $repl = array($config['home_title'], $config['http_home_url'], $title, cute_get_link(array('id' => $id, 'date' => $added_time, 'title' => $title, 'category' => $category, 'url' => $url)), $short_story);
...
global $output, $row, $config, $allow_full_story, $xfields;
	...
	$output = preg_replace('/<lj (.*?)=(\\\"|"|\'{0,1})(.*?)(\\2)>/i', '<a href="http://\\3.livejournal.com/profile/"><img height="17" src="'.$config['http_script_dir'].'/skins/images/\\1.gif" width="17" align="absmiddle" border="0" alt="[info]"></a><a href="http://\\3.livejournal.com/">\\3</a>', $output);
