...
$that = '-'; // чем заменять пробел в УРЛах
...
<li><a href="<?=$PHP_SELF; ?>?action=users"><?=t('Пользователей'); ?>*</a>
...
<li><a href="<?=$PHP_SELF; ?>?action=categories"><?=t('Категории'); ?></a>
...
<li><a href="<?=$PHP_SELF; ?>?action=news"><?=t('Новости'); ?></a>
...
<li><a href="<?=$PHP_SELF; ?>?action=comments"><?=t('Комментарии'); ?></a>
...
<li><a href="<?=$PHP_SELF; ?>?action=xfields"><?=t('Дополнительные поля (XFields)'); ?></a>
...
<li><?=t('Комментарии'); ?>**
<li><?=t('Дополнительные поля (XFields)'); ?>**

<?
}
?>

<p>* <?=t('у тех, что есть (со схожим логином) всё будет перенесено из старой базы, кроме паролей'); ?>
<p>** <?=t('сналало нужно восстановить базу новостей'); ?>
<p><?=t('Будьте осторожны при конвертации баз. Написано всё на коленке, если есть возможность, то конвертируйте базу на локальном сервере, ибо там оперативки много, а конвертер жрёт её только так.'); ?>